...
Creates a hosted Foundry agent with two tools:
  1. MicrosoftFabricPreviewTool – routes supply-chain DATA inquiries to the Fabric data agent
...
SYSTEM_INSTRUCTIONS = """You are the Supply Chain Orchestrator Agent.
...
- Demand forecasts, production schedules
- Any structured data lookup from the supply-chain database

## Intent: POLICY_KNOWLEDGE
Use the **Azure AI Search** tool when the user asks about:
- Supply-chain policies, SOPs, compliance rules
- Procurement guidelines, vendor onboarding procedures
- Sustainability or ESG policies
...
def create_orchestrator_agent():
    ...
    with tracer.start_as_current_span("create_orchestrator_agent"):
        ...
        # --- Tool 1: Microsoft Fabric Data Agent (supply-chain data queries) ---
